...
#define SIZE_OF_INPUT_DATA 1000	//bytes
...
class Q2cacheTest: public ::testing::Test {
	...
	protected: 
		void SetUp() {
			...
			printf("Fifo_bf_test:%s\n", fifo_bf_test);
			fifo_sz_cur = 0;														//fifo sz after inserting pages we get
			...
			lru_sz_exp = 0;															//lru sz after inserting pages we expect
			...
			lru_sz_cur = 0;															//lru sz after inserting pages we get
